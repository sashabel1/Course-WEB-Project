  ...
  return (
    <div className="flex justify-center items-start gap-[30px] p-5 w-full max-w-[1400px] box-border flex-wrap lg:flex-nowrap">
      ...
      <div className="flex-1 min-w-0">
        {fullText ? (
          <div className="w-full bg-white p-6 rounded-lg shadow-[0_10px_15px_rgba(0,0,0,0.1)] border border-gray-200 box-border">
            <h2 className="text-[2rem] font-bold text-[#006A71] mb-4 text-center">
              {`Results for "${query}"`}
              {(startYear || endYear) && (
            ...
            <details className="my-4 border border-gray-300 rounded-lg p-2 bg-[#f9f9f9]">
              <summary className="cursor-pointer font-bold text-[#0077cc] hover:underline">
                Show Wikipedia summary
              </summary>
              <p className="mt-2 leading-6 whitespace-pre-wrap text-gray-700 max-h-[240px] overflow-y-auto border border-gray-300 p-3 rounded bg-[#f9fafb]">
                {fullText}
              </p>
            </details>
            ...
            {timelineEvents.length > 0 ? (
              <div className="relative px-[50px] py-5">
                <div className="absolute top-0 bottom-0 left-1/2 w-1 bg-[#006A71] -translate-x-1/2 z-0" />
